...
def test_delete_user(client, test_superuser, superuser_token_headers):
    response = client.delete(
        f"/api/v1/users/{test_superuser.id}", **superuser_token_headers
    )
    assert response.status_code == 200
...
def test_delete_user_not_found(client, superuser_token_headers):
    response = client.delete("/api/v1/users/4321", **superuser_token_headers)
...
def test_get_user(client, test_user, superuser_token_headers):
    response = client.get(
        f"/api/v1/users/{test_user.id}", **superuser_token_headers
    )
    assert response.status_code == 200
    assert response.json() == {
        "id": test_user.id,
...
def test_user_not_found(client, superuser_token_headers):
    response = client.get("/api/v1/users/123", **superuser_token_headers)
    assert response.status_code == 404


def test_authenticated_user_me(client, user_token_headers):
    response = client.get("/api/v1/users/me", **user_token_headers)
    assert response.status_code == 200


def test_unauthenticated_routes(client, db):
...
def test_unauthorized_routes(client, user_token_headers):
    ...
    response = client.get("/api/v1/users/123", **user_token_headers)
    assert response.status_code == 403
